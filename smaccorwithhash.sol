...
// ----------------------------------------------------------------------------
/// @title Securable : Secure a contract from outsiders unauthorized calls.
// ----------------------------------------------------------------------------
abstract contract Securable {
    ...
    address public founder = PPeC(PPeCAddress).founder(); // PPeC founder address.
    ...
    function checkHash(string memory key, bytes32 newHash)
    public
    returns (bool)
    {
        // Hashing the key for verification.
        ...
        // Revert the call if the hash is not an approved hash.
        if (senderHash[msg.sender][oldHash] != true)
            revert UnapprovedHash();

        // unset sender old approved hash.
        senderHash[msg.sender][oldHash] = false;
        // Set sender new approved hash.
        senderHash[msg.sender][newHash] = true;
        return true;
    }
...
// ----------------------------------------------------------------------------
contract AdCreator is Securable {
...
interface SmACCor {
    ...
    /// Update the promoter pledged balance [SmAC contracts calls only].
    function checkHash(string memory key, bytes32 newHash) external returns(bool);
...
contract SmartAd {
    ...
    public
    {
        ...
        // Then set a new hash.
        SmACCor(adCreatorAddress).checkHash(key, newHash);    
